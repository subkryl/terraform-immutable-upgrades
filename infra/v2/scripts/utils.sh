...
function tf_get_instance_id() {
	...
	local instance=${1}
	local id
	id=$(echo ${TFSTATE} | jq -e -r -M '.modules[0].resources."aws_instance.'"${instance}"'".primary.id')
	if [ $? -ne 0 ]; then
		# if someone has tainted the resource try with tainted instead of primary
		id=$(echo ${TFSTATE} | jq -e -r -M '.modules[0].resources."aws_instance.'"${instance}"'".tainted[0].id')
		if [ $? -ne 0 ]; then
			echo ""
			return
		fi
